...
class NaverOAuthProvider(
    ...
    override fun getAuthorizationUrl(redirectUri: String): String {
        ...
        // 상태 저장
        stateStore.saveState("naver", state, 10, TimeUnit.MINUTES)
    ...
    override fun getAccessToken(code: String): String {
        try {
            ...
            log.error { "네이버 액세스 토큰 획득 실패: ${e.message}" }
    ...
    override fun getUserInfo(accessToken: String, includeProfileImage: Boolean): UserInfo {
        try {
            ...
            log.debug { "네이버 사용자 정보 응답: $userInfo" }

            // 사용자 정보 추출
            val email = userInfo.path("response").path("email").asText()
            val name = userInfo.path("response").path("name").asText()
            val providerId = userInfo.path("response").path("id").asText()

            // 프로필 이미지는 요청된 경우에만 추출
            val profileImageUrl = if (includeProfileImage) {
                userInfo.path("response").path("profile_image").asText("").takeIf { it.isNotBlank() }
            } else {
                null
            }

            return if (email.isBlank() || providerId.isBlank()) {
                log.error { "필수 네이버 사용자 정보 누락 - 이메일: ${email.isBlank()}, 공급자ID: ${providerId.isBlank()}" }
                throw ApplicationException(ErrorCode.EXTERNAL_API_ERROR)
            } else {
                UserInfo(
            ...
            log.error { "네이버 사용자 정보 획득 실패: ${e.message}" }
            throw ApplicationException(ErrorCode.EXTERNAL_API_ERROR)
        }
    }
    ...
    override fun validateCallback(params: Map<String, String>, request: HttpServletRequest): Boolean {
        ...
        return stateStore.validateState("naver", state)
    }

    override fun getProviderType(): Provider = Provider.NAVER
}
